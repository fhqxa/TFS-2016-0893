...
function r=dependency_s_gs_treeok(data_array,delta,k,tree)
treeParent=tree(:,1)';
% treeLevel=tree(:,2)';
clear tree;
...
%          array_diff(j,:)=min(array_diff(j,:)-data_array(i,1:n-1),1);%%%%%%%%%% for  nominal attributes
      ...
      %  r=r+(1-(exp(-value_nearest/2/s^2)))/m;% Written by HQH
       ...
       if(len==0)
        ...
        array_diff=data_array(label_diff,1:n-1);
        [p,q]=size(array_diff);
        array_diff1=array_diff;
        for j=1:p
            array_diff1(j,:)=min(abs(array_diff(j,:)-data_array(i,1:n-1)),1);%%%%%%%%%% for  nominal attributes
        end
        d=array_diff1.^2;
        [x,y]=sort(sum(d,2));
        if(length(y)<k)
            temp=median(array_diff(y(1:1),:));
        else
